...
//***
	...
	public function woof_draw_checkbox_childs_golmaal($taxonomy_info, $tax_slug, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $is_first = 'N') {
		...
		$shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) ); 
    	if (!empty($childs) AND is_array($childs)) {
    		foreach ($childs as $k => $term) {  $inique_id = uniqid();
                $count_string = "";
                $count = 0;
                $this->tests =array();
		        $uniquechilds =array();
		        foreach ($term['childs'] as  $child) {
		            $this->tests[]= $child['slug'];
		            if(count($child['childs'])>0 && isset($child['childs'])) {
		                $this->adfadsfadsf($child);
		            }
		        }
		        $uniquechilds = array_unique($this->tests);
		        if (in_array($current_request[0], $uniquechilds) || empty($current_request[0])) { ?>
		            <script type="text/javascript"> 
		            shownLis.push('<?php echo $term['term_id']; ?>');
		            </script>
		            <?php 
		        } 
		        end($childs);
 ...
 ?> 
        	 		<ul class="expand_button_<?php echo $term_id; ?>" 
        	 			style="<?php echo (in_array($current_request[0], $uniquechilds) || $current_request[0] == $term['slug']) || ($term['is_first'] == 'Y' && empty($current_request[0]))?"display:block;":"display:none;"; ?>">
        	 			<li class="container" <?php echo $style; ?>>
						<?php if(!empty($term['childs'])) { ?>
							<?php if((in_array($current_request[0], $uniquechilds) && count($term['childs'])<=1) || ($current_request[0] == $term['slug'] && empty($term['childs'])) ) { ?>
							<a href="javascript: void(0);" class="expand_button poof_is_opened" id="expand_button_<?php echo $term['term_id']; ?>">-</a>
							<?php } else { ?>
							<a href="javascript: void(0);" class="expand_button poof_is_closed" id="expand_button_<?php echo $term['term_id']; ?>">+</a>
							<?php } ?>
						<?php } ?>
						 <a href="<?php echo $shop_page_url; ?>?swoof=1&product_cat=<?php echo $term['slug']; ?>"><?php echo $term['name']; ?></a>
						 	<?php if (!empty($term['childs']))
    ...
    public function adfadsfadsf($childsa) {
		    foreach ($childsa['childs'] as  $childa) { //echo "<pre>";print_r($childa);
	    	// echo $childa['slug'];
	        
	            if(count($childa['childs'])>0) {
	            	$this->tests[] = $childa['slug'];
	               $this->adfadsfadsf($childa);
	            } else {
	            	$this->tests[] = $childa['slug'];
	            }
	        }
