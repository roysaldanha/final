...
//***
	...
	public function woof_draw_checkbox_childs_golmaal($taxonomy_info, $tax_slug, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos) {
    	...
    	if (!empty($childs) AND is_array($childs)) {
    		foreach ($childs as $k => $term) {  $inique_id = uniqid();
		        ...
		        if($key == $k) {
		        	$style = 'style="background: url(\'http://www.mothsofindia.org/graphics/dottedangle.gif\') no-repeat scroll 1px 0.9em transparent;"';
		        } else {
 ...
 ?> 
        	 		<ul class="expand_button_<?php echo $term_id; ?>" 
        	 			style="<?php echo in_array($current_request[0], $uniquechilds) || empty($current_request[0]) || $current_request[0] == $term['slug']?"display:block;":"display:none;"; ?>">
        	 			<li class="container" <?php echo $style; ?>>
						<?php if(!empty($term['childs'])) { ?>
							<?php if(in_array($current_request[0], $uniquechilds) || empty($current_request[0]) || $current_request[0] == $term['slug']) { ?>
							<a href="javascript: void(0);" class="expand_button poof_is_opened" id="expand_button_<?php echo $term['term_id']; ?>">-</a>
							<?php } else { ?>
